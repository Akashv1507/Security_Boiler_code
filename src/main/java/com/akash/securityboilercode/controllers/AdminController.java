...
public class AdminController {
	...
	public String getAllEmployees(Model model) {
		ArrayList<EmployeesVm> employeesVm = new ArrayList<>();
		Authentication authentication = authenticationWrapperService.getAuthentication();
		List<Employee> employees = employeeRepository.findAll();

		for ( Employee emp : employees) {
			List<String> empRoleStrings = new ArrayList<>();
			emp.getEmpRoles().stream().forEach(empRole-> empRoleStrings.add(empRole.getRoleName()));
			employeesVm.add(new EmployeesVm(emp.getId() ,emp.getEmpId(), emp.getEmail(), emp.getName(), empRoleStrings));
		}
